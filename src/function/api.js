export function getSearchApi(search, type) {
    ...
    return fetch(`https://collectionapi.metmuseum.org/public/collection/v1/search?medium=${type}&q=${search}`)
        .then(r => r.json())
        .then((recurso) => {
            if (recurso.total === 0) {
                ...
                if (idResultados.length > 20) { idResultados = idResultados.slice(0, 20) }
...
export function getSearchForArtistApi(search) {
    ...
    return fetch(`https://collectionapi.metmuseum.org/public/collection/v1/search?artistOrCulture=true&q=${search}`)
        .then(r => r.json())
        .then((recurso) => {
            if (recurso.total === 0) {
                alert('No se encuentras coincidencias')
